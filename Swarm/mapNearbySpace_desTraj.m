...
function [X,Y,Vx,Vy]=mapNearbySpace_desTraj(k1,k2,kv,kf,b,c,xgoal,obstacle)
    ...
    ICsize1=2; ICsize2=2;
    X0=ICsize1*rand(1,N);   % Pick random values for initial positions in X and Y dimensions
    Y0=ICsize1*rand(1,N);
...
%         vbar=mean([Vx(end,:)' Vy(end,:)']);      % and for velocity also
        ...
        ux(1,:)=-k1*(X(end,:)-mean(X(end,:))) - k2*(Vx(end,:)-mean(Vx(end,:))) - kv*Vx(end,:) + xrepel - kf*(A(:,1)');
